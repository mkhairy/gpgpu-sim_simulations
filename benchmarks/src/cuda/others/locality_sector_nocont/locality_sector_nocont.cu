...
__global__ void locality_sector_nocont(const float* A, const float* B, float* C, int N)
...
void VectorAddition(int N, int threadsPerBlock, int compute, int scale)
{
	...
	locality_sector_nocont<<<blocksPerGrid, threadsPerBlock>>>(d_A, d_B, d_C, N);
